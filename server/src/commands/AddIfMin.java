...
public class AddIfMin implements Command {
    ...
    public CommandResponse execute(Manager manager, Map<String, Object> args, int userId) {
        ...
        Ticket minTicket = manager.getSnapshot().stream()
                .min(Ticket::compareTo)
                .orElse(null);

        if (minTicket == null || ticket.compareTo(minTicket) < 0) {
            try {
            ...
            return new CommandResponse("Элемент не добавлен (не минимальный)");
        }
    }
}
